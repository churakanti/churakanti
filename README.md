# Hi, I'm Siri Ch 👋

MS Computer Science (Intelligent Systems) · Lawrence Technological University

🔬 First-author IEEE publication on evolutionary hyperparameter optimization  
🎯 98.97% accuracy on computer vision regression (ResNet50 + EfficientNet)  
🤖 Building: Deep Learning · Computer Vision · LLM Agents  
📍 Southfield, MI · Open to ML Engineer & Computer Vision roles  
🛂 3-year OPT work authorization (F-1 STEM eligible)

### 🔗 Links
- 🌐 [Portfolio](https://churakanti.github.io)
- 📄Optimizing Hyperparameters for Deep Learning Models Using Evolutionary Algorithms: Solving the Four-Class Intertwined Spiral Classification Problem
   [IEEE Paper](https://ieeexplore.ieee.org/document/11103665)

